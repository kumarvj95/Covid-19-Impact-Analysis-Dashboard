...
app.layout =html.Div([
    ...
    html.Div([
        html.Div([
            html.Div([
                html.Div([
                    dcc.Dropdown(id='plot-graph',options=options1,value='All'),#graph k liye dcc lgaya
                    dcc.Graph(id='graph')
                ],className='card-body')
            ],className='card bg-success') #bg-sucess means background color green ho gaya
        ],className='col-md-6'),
        html.Div([
            html.Div([
                html.Div([
                    dcc.Dropdown(id='my_dropdown',options=options2,value='Status'),
                    dcc.Graph(id='the_graph') # maan se naam de diya
                ],className='card-body')
            ],className='card bg-info')
        ],className='col-md-6')
    ],className='row'),
...
def generate_graph(type):
    ...
    if type=='All':
        return {'data':[go.Line(x=patients['Status'],y=patients['Total'])], #'go.' means plotly.graph
